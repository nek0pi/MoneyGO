...
{
    [Header("Attributes")] [Tooltip("Amount to increment each object's position every FixedUpdate")] [SerializeField]
    float incrementX = 5f;

    [Tooltip("Lowest object is reset to the top when this many increments is reached")] [SerializeField]
    float resetAt = 200f;
    ...
    void FixedUpdate()
    {
        ...
        foreach (GameObject obj in toScroll)
        {
            ...
            (
                ...
                obj.transform.position.x + incrementX,
                obj.transform.position.y 
            );
            // Output newPosition vector to the transform position
            obj.transform.position = newPosition;
            runningIncrement += incrementX;
